...
# 当 OCR 子进程因 torch 崩溃(env 退化)反复失败时, 短时禁用 OCR, 避免每次验证码都傻等崩溃+黑框闪烁
_OCR_DISABLED_UNTIL = 0.0

def _ocr_readtext_subprocess(image):
    """在【隔离子进程】跑 EasyOCR, 返回 [(bbox, text, prob)] 或 None.
    设计目的: 把 torch/EasyOCR 这条会原生崩溃(Access Violation)的依赖关在子进程里,
    即使 torch 加载崩了也只有子进程死, 主服务(及要求1的数据返回)不受影响;
    验证码识别失败时主服务优雅降级(返回 None), 不会拖垮进程.
    torch 在本环境恢复正常后, 无需改代码即可自动恢复验证码识别.

    行为增强:
    - 子进程以 CREATE_NO_WINDOW 启动, 不再弹出 cmd 黑框.
    - 若子进程崩溃(典型: torch 在本环境加载即 0xC0000005), 5 分钟内直接跳过 OCR,
      查询秒回(验证码无法自动解, 但不再被崩溃循环拖累). 冷却到期后自动重试,
      torch 恢复即自动复活."""
    global _OCR_DISABLED_UNTIL
    if _t.time() < _OCR_DISABLED_UNTIL:
        return None  # OCR 已短时禁用(环境退化), 跳过避免反复崩溃
    try:
        import os as _os
        ...
        fd, tp = _tf.mkstemp(suffix=".png")
        _os.close(fd)
        try:
            im.save(tp)
            _helper = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "ocr_helper.py")
            p = _sp.run([sys.executable, _helper, tp],
            ...
            try:
                ...
                pass
        if p.returncode != 0:
            _err = (p.stderr or "") + (p.stdout or "")
            # torch 崩溃特征: 0xC0000005 / Access Violation / torch 关键字 -> 短时禁用, 不再反复傻等
            if ("0xC0000005" in _err) or ("Access Violation" in _err) or ("torch" in _err.lower()):
                _OCR_DISABLED_UNTIL = _t.time() + 300.0
                print(f"[warn] OCR 子进程崩溃(疑似 torch 环境退化 rc={p.returncode}), 5 分钟内跳过 OCR 自动解卡: {_err[-300:]}")
            else:
                print(f"[warn] OCR 子进程异常退出 rc={p.returncode}: {_err[-300:]}")
            return None
        out = (p.stdout or "").strip().splitlines()
        if not out:
            return None
        try:
            data = _json.loads(out[-1])
        except Exception:
            return None
        if not data.get("ok"):
            return None
        ...
        for w in data.get("words", []):
            bbox = [(pt[0], pt[1]) for pt in w[0]]
            res.append((bbox, w[1], w[2]))
        return res
    except Exception as e:
...
def health():
    return jsonify({
        ...
        "port": PORT,
        "ocr_ready": os.path.exists(os.path.join(os.path.dirname(os.path.abspath(__file__)), "ocr_helper.py")),
        # EasyOCR 现为子进程隔离 + 惰性: ocr_ready 表示识别助手可用(验证码将在子进程内真实加载 torch);
        # 若子进程内 torch 原生崩溃, 该次识别优雅失败, 不影响主服务与数据查询.
    })
